...
message LimitOffsetRequest {
  int32 limit = 1  [(validate.rules).int32 = {gt: 0, lte: 100}]; // max 100 rows per request - the API clamps to DB_MAX_ROWS
  int32 offset = 2 [(validate.rules).int32 = {gte: 0}];
}

message PaginationRes {
  int32 limit = 1       [json_name = "limit"];      // limit actually applied - the server clamps to DB_MAX_ROWS
  int32 offset = 2      [json_name = "offset"];
  int64 total = 3       [json_name = "total"];      // total rows matching the query, ignoring limit/offset
  bool has_more = 4     [json_name = "hasMore"];
  int32 next_offset = 5 [json_name = "nextOffset"]; // only meaningful when has_more is true
}
...
message Match {
  ...
  double qty1 = 6            [json_name = "qty1"]; // qty matched by the clob
  double qty2 = 7            [json_name = "qty2"]; // qty matched by the clob
...
message MarketResponse {
  ...
  bool is_lom_enabled = 21         [json_name = "isLOMenabled"];
}
